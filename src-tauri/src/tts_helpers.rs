...
impl TextToSpeech {
    ...
    fn _infer(
        ...
        // Convert ndarray -> (shape, flat_vec) because ort::Value::from_array
        // in this environment accepts (shape_vec, data_vec) tuples.
        let text_ids_flat: Vec<i64> = text_ids_array.iter().cloned().collect();
        let text_ids_shape_vec: Vec<usize> = text_ids_array.shape().to_vec();
        let text_ids_value = Value::from_array((text_ids_shape_vec, text_ids_flat))?;

        let text_mask_flat: Vec<f32> = text_mask.iter().cloned().collect();
        let text_mask_shape_vec: Vec<usize> = text_mask.shape().to_vec();
        let text_mask_value = Value::from_array((text_mask_shape_vec, text_mask_flat))?;

        let style_dp_flat: Vec<f32> = style.dp.iter().cloned().collect();
        let style_dp_shape_vec: Vec<usize> = style.dp.shape().to_vec();
        let style_dp_value = Value::from_array((style_dp_shape_vec, style_dp_flat))?;
        ...
        // Encode text
        let style_ttl_flat: Vec<f32> = style.ttl.iter().cloned().collect();
        let style_ttl_shape_vec: Vec<usize> = style.ttl.shape().to_vec();
        let style_ttl_value = Value::from_array((style_ttl_shape_vec, style_ttl_flat))?;
        ...
        let total_step_vec: Vec<f32> = vec![total_step as f32; bsz];
        let total_step_value = Value::from_array((vec![bsz], total_step_vec))?;
 
         // Denoising loop
         for step in 0..total_step {
            let current_step_vec: Vec<f32> = vec![step as f32; bsz];
            let current_step_value = Value::from_array((vec![bsz], current_step_vec))?;

            let xt_flat: Vec<f32> = xt.iter().cloned().collect();
            let xt_shape_vec: Vec<usize> = xt.shape().to_vec();
            let xt_value = Value::from_array((xt_shape_vec, xt_flat))?;

            let text_emb_flat: Vec<f32> = text_emb.iter().cloned().collect();
            let text_emb_shape_vec: Vec<usize> = text_emb.shape().to_vec();
            let text_emb_value = Value::from_array((text_emb_shape_vec, text_emb_flat))?;

            let latent_mask_flat: Vec<f32> = latent_mask.iter().cloned().collect();
            let latent_mask_shape_vec: Vec<usize> = latent_mask.shape().to_vec();
            let latent_mask_value = Value::from_array((latent_mask_shape_vec, latent_mask_flat))?;

            // reuse previously created text_mask_value and total_step_value
        ...
        // Generate waveform
        let final_latent_flat: Vec<f32> = xt.iter().cloned().collect();
        let final_latent_shape_vec: Vec<usize> = xt.shape().to_vec();
        let final_latent_value = Value::from_array((final_latent_shape_vec, final_latent_flat))?;
