...
async function generateStream(params: GenerateCompletionStreamParams, callback?: (chunk: string) => void): Promise<number[]> {
  let fullResponse = '';
  let unlisten: () => void;
...
async function generateEmbeddingsBatch(
  ...
  try {
    for (let i = 0; i < params.texts.length; i += BATCH_SIZE) {
      ...
      console.log(`Generating embeddings for batch ${i / BATCH_SIZE + 1}/${Math.ceil(params.texts.length / BATCH_SIZE)}`);
