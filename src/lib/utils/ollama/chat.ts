...
const DEFAULT_BASE_URL =
	typeof import.meta !== "undefined" && import.meta.env?.OPENAI_BASE_URL
...
function adaptRequestOptions(
	...
	if (request.options?.presence_penalty !== undefined)
		params.presence_penalty = request.options.presence_penalty
	if (request.options?.frequency_penalty !== undefined)
		params.frequency_penalty = request.options.frequency_penalty
...
export async function createBatchChat(
	...
	try {
		...
		if (error instanceof OpenAI.APIError) {
			throw new ChatAPIError(`API error: ${error.message}`, error.status, error)
		}
 ...
 */
export async function chat(request: ChatRequest, baseUrl?: string): Promise<ChatResponse> {
			...
			for (const deltaToolCall of chunk.choices[0].delta.tool_calls as any[]) {
				...
				if (existing) {
					...
					if (deltaToolCall.function?.arguments) {
						existing.arguments = (existing.arguments || "") + deltaToolCall.function.arguments
					}
