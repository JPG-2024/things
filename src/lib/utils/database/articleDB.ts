...
export async function getOrCreateMainColor(articleId: number): Promise<string> {
	...
	try {
		...
		await db.execute(`UPDATE articles SET mainColor = $1 WHERE rowid = $2`, [
			calculatedColor,
			articleId,
		])
...
export async function updateArticleEmbeddingsStatus(
	articleId: number,
	embeddings: boolean,
): Promise<void> {
	const db = await getDb()
	...
	try {
		await db.execute(`UPDATE articles SET embeddings = $1 WHERE rowid = $2`, [
			embeddings,
			articleId,
		])
	} catch (error) {
