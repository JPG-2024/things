...
function mergeToolCallDelta(target: LlamaChatToolCall, delta: LlamaChatToolCall): LlamaChatToolCall {
	return {
...
export async function chatCompletions(
	...
	if (!res.ok) {
		...
		throw new LlamaChatCompletionError(`llama-server /v1/chat/completions failed: ${message}`, res.status, payload);
	}
	...
	const created = (lastChunk as LlamaChatCompletionsChunk | null)?.created ?? Math.floor(Date.now() / 1000);
